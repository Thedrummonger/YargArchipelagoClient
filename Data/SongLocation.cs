...
namespace YargArchipelagoClient.Data
{
    public class SongLocation(int num)
    {
        ...
        public bool SongAvailableToPlay(ConnectionData connection) =>
            SongItemReceived(connection) && HasUncheckedLocations(connection);
        public bool SongItemReceived(ConnectionData connection) =>
            connection.ReceivedSongs.Contains(SongNumber);
        public bool IsGoalSong(ConfigData config) =>
            config.GoalSong.SongHash == SongHash;
    }
}
